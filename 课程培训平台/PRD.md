...
#### 3.1.4 我的订单
   ...
   - 状态展示映射：`已支付` 展示为「待核销」，`已核销` 展示为「已生效」；小程序订单支付成功后自动核销，`已支付` 状态极短，小程序端不展示「待核销」，该展示仅针对线下订单
...
#### 3.1.5 我的课期
   ...
   - 仅展示已核销生效且当前仍持有有效名额的课期列表（按时间倒序）；包含受赠获得的课期名额，与购买名额同等展示
   - 课期状态：待上课、进行中、已完成
...
1. 工作人员在后台生成课期签到二维码
2. 学员使用微信/小程序扫描课期二维码，服务端为其锁定一个可签到的有效名额，生成个人专属签到二维码（含 registration_slot_id、学员 ID、时间戳）
...
- **上课信息字段**：抖音号、当前月收入、学习目标、备注、客户来源（直播间加好友/别人介绍，由业务员填写）、信息完善时间
- **手动添加**：运营人员可手动录入客户（用于线下转化场景）
- **业务员绑定**：每个客户可绑定一个业务员，用于业务员业绩统计；客户注册时可选择绑定业务员（非必填），注册后管理员/运营可随时在后台为客户绑定或更换业务员

#### 3.2.1.1 课程分类管理

- **分类列表**：展示所有课程分类（名称、排序、关联课程数）
- **操作**：新增、编辑、删除（无关联课程时）、排序
- **说明**：分类为后台可配置项，运营人员可自行增删改
...
- 字段：课程 ID、课程名称、课程简介、封面图、创建人、创建时间、状态（草稿/上架/下架）、归属类别（关联 3.2.1.1 中的分类）、排序优先级、课程总收入（该课程下所有已核销订单的金额汇总，自动更新）
- 操作：新增、编辑、上架、下架、删除（无关联课期时）
- 状态说明：`草稿` 为创建后未发布状态，仅在后台可见；`上架` 对小程序用户可见可购买；`下架` 停止对外展示，不影响已报名课期
...
- **业务员线下充值**：业务员为客户手动创建充值订单，填写客户实际付款时间（复用 `paid_at` 字段，记录客户实际转账/付款的时间）、付款订单号、付款截图等，直接标记为已支付（现金/转账已收），等待财务核销
- **财务核销**：线下订单（购课/充值）支付成功后状态为已支付，财务人员核销后变为已核销，同时创建报名汇总和名额明细；小程序购课与充值订单支付成功后自动核销并创建报名汇总和名额明细
- **退款处理**：仅支持退订单下仍归属于当前客户、未转赠、未分配座位且无任何签到记录（CheckInRecord）的剩余名额；**通过转赠获得的名额不可退款**（仅原始购买人可对其名下未转赠名额发起退款），已转赠部分不可退款
...
- **账号字段**：用户名、显示名称、手机号、微信号、微信昵称、角色、状态（启用/禁用）
...
- 支付超时：超过 `payment_deadline_at` 自动关闭订单，避免脏订单长期占用资源。
...
stateDiagram-v2
    ...
    note right of 已拒绝
        线下：可修改重提
        如需退已收现金/转账，走线下人工退款+日志记录，不走系统电子退款通道
    end note
...
| refund_no | 退款单号，唯一 |
| order_id | 关联订单ID |
| applicant_customer_id | 申请客户ID（客户申请时必填，后台发起时也必填指定客户） |
| source | 退款来源：客户申请/后台发起 |
| type | 类型：全额/部分 |
| channel | 渠道：原路退回/线下处理 |
| status | 退款状态：待审核/已批准/已拒绝/已完成/已取消 |
| amount | 退款金额 |
| reason | 退款原因 |
| rejected_reason | 拒绝原因（展示给前端） |
| handled_by | 财务处理人ID |
| handled_at | 处理时间 |
| idempotency_key | 退款幂等键（order_id-applicant_customer_id-时间戳） |
| created_at | 退款时间 |
| updated_at | 更新时间 |
...
- 个人签到码有效期可配置（默认 120 秒），完成签到核验后立即失效，防截图转发；签到码过期后该名额的锁定状态不释放，学员需重新扫码生成新签到码
...
- 支持部分转赠、多次转赠（不限制次数）
- 仅 `RegistrationSlot.status = 有效`、`gift_status = 可转赠`（即未转赠中、未已转赠）且未分配座位的名额可转赠
- 业绩始终归属最开始的业务员（Registration.sales_agent_id 和 RegistrationSlot.sales_agent_id 不变）
- 转赠通过分享链接发起，受赠人点击确认后生效
- **分享链接安全**：转赠链接内含 `share_token`（32 位加密安全随机字符串，取自 `gift_records.share_token`）；链接被受赠人确认后立即失效（`gift_records.accepted_at` 标记），同一链接不可重复使用；确认后名额 `gift_status` 置为「已转赠」，`gift_level = 1`
- **防暴力猜测**：`share_token` 使用 `crypto/rand` 级别随机源生成，有效空间 ≥ 2^128，服务端对无效 token 访问做频次限制（单 IP 每分钟 ≤ 10 次）
...
- **转赠过期/取消**：转赠链接有效期 24 小时（`gift_records.expires_at`），过期后 `gift_records.status = expired`，名额 `gift_status` 恢复为可转赠；发起人可在受赠人确认前主动取消转赠（`gift_records.status = cancelled`），名额 `gift_status` 同步恢复为可转赠
...
- 退款金额按可退款名额数量 × 订单创建时的单价快照（orders.unit_price_amount）计算
- 转赠后仅支持退剩余未转赠部分（如购买5个名额，转赠2个，只可退3个）
- 线下订单退款由工作人员处理，记录日志即可
- **幂等控制**：同一订单同一申请人同一时间窗口内仅允许一笔 `pending` 状态的退款申请；服务端以 `order_id + applicant_customer_id` 作为幂等键，5 分钟内重复请求直接返回已有退款单状态
- **工作人员代客退款**：后台工作人员发起退款时，必须指定 `客户` 作为退款申请人（`refunds.applicant_customer_id` 必填），退款名额筛选规则与客户自行申请完全一致；工作人员不可对自己名下的任意名额发起退款
- **转赠名额不可退款**：通过转赠获得的名额（`registration_slots.owner_customer_id ≠ registration_slots.root_customer_id`）不可退款；仅原始购买人（`root_customer_id`）可对其名下未转赠、未分配座位、无签到记录的名额发起退款
...
flowchart TD
    ...
    D -->|否| E[直接关闭订单]
    D -->|是| F[筛选可退款名额<br/>root_order_id=当前订单<br/>且 owner_customer_id=申请人（仅原始购买人可退）<br/>且 root_customer_id=申请人（仅原始购买人可退）<br/>且未转赠、未分配座位、gift_level=0、gift_status=0<br/>且 status=有效<br/>且不存在 CheckInRecord]
    F --> G{可退款名额>0?}
    G -->|否| H[返回不可退款]
    G -->|是| I[按可退款名额计算退款金额]
    I --> J{订单类型}
...
**业务员绑定**
- 客户注册时可绑定业务员（非必填），注册后管理员/运营可在后台为客户绑定或更换业务员
- 业绩归属以订单创建时客户绑定的业务员为准

**课程分类管理**
- 分类为后台可配置项，运营人员可自行增删改、排序
- 删除分类时若存在关联课程则禁止删除
...
- 签到码有效期：可配置，默认 120 秒；码过期后名额锁定状态不释放，学员需重新扫码
